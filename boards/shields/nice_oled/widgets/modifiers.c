...
// Helper para limpiar animación previa si cambiamos de modificador
static void clear_luna_anim(void) {
    if (luna_imgs) {
        lv_obj_del(luna_imgs);
        luna_imgs = NULL;
    }
...
static void create_luna_anim(lv_obj_t *parent, const lv_image_dsc_t **imgs) {
    // Si ya hay una animación, la borramos para crear la nueva (ej. cambio de Walk a Run)
    clear_luna_anim();

    luna_imgs = lv_animimg_create(parent);
    lv_obj_center(luna_imgs);
    
    // Casting a (const void **) es necesario para la API de animimg
    lv_animimg_set_src(luna_imgs, (const void **)imgs, 2);
...
static void set_modifiers_text(lv_obj_t *label, struct modifiers_state ignored) {
    ...
    // Limpiamos texto porque usamos el perro
    lv_label_set_text(label, "");

    if (mods & (MOD_LGUI | MOD_RGUI)) {
        // Solo creamos si no es la misma animación ya corriendo
        // (Nota: para simplicidad, recreamos si cambia el estado de modifiers)
        create_luna_anim(label, luna_imgs_sit_90);

    } else if (mods & (MOD_LALT | MOD_RALT)) {
        create_luna_anim(label, luna_imgs_walk_90);

    } else if (mods & (MOD_LCTL | MOD_RCTL)) {
        create_luna_anim(label, luna_imgs_run_90);

    } else if (mods & (MOD_LSFT | MOD_RSFT)) {
        create_luna_anim(label, luna_imgs_sneak_90);

    } else {
        // Si no hay modificadores, quitamos al perro
        clear_luna_anim();
    }
}
