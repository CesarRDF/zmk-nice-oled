...
static void draw_canvas(lv_obj_t *widget, lv_color_t cbuf[],
  ...
  // LOG_INF("draw_canvas: INICIO"); // Descomentar si se necesita mucho
  // detalle, puede saturar
...
int zmk_widget_screen_init(struct zmk_widget_screen *widget, lv_obj_t *parent) {
  LOG_INF("zmk_widget_screen_init: INICIO");
  ...
  LOG_INF("zmk_widget_screen_init: Configurando buffer de canvas");
  lv_canvas_set_buffer(canvas, widget->cbuf, CANVAS_HEIGHT, CANVAS_HEIGHT,
                       LV_COLOR_FORMAT_I1);

  sys_slist_append(&widgets, &widget->node);

  LOG_INF("zmk_widget_screen_init: Dibujando animacion inicial");
  draw_animation(canvas, widget);

  LOG_INF("zmk_widget_screen_init: Inicializando widgets hijos");
  widget_battery_status_init();
  widget_peripheral_status_init();

  LOG_INF("zmk_widget_screen_init: FIN");
  return 0;
}
