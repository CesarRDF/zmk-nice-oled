...
#define SRC(array)                                                             \
  (const void **)array, sizeof(array) / sizeof(lv_image_dsc_t *)
...
static void set_animation(lv_obj_t *animing,
                          struct luna_wpm_status_state state) {
  // Lógica de cambio de animación basada en WPM
  if (state.wpm < 15) { // Estaba en 5, subido a 15
