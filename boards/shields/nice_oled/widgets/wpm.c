...
// LVGL 9 prefiere LV_IMAGE_DECLARE, pero mantenemos compatibilidad si es
// posible
 ...
 * ------------------------------------------------------------------------- */
static void draw_image_on_canvas(lv_obj_t *canvas,
                                 const lv_image_dsc_t *img_src, int x, int y) {
  lv_layer_t layer;
...
static void draw_needle(lv_obj_t *canvas, const struct status_state *state) {
  ...
  line_dsc.color =
      lv_color_white(); // O lv_color_black() según tu tema (LVGL_FOREGROUND)
  ...
  // float angleRad = angleDeg * (3.14159 / 180.0f); // Simplificamos para
  // evitar warnings de float
...
static void draw_label(lv_obj_t *canvas, const struct status_state *state) {
  ...
  label_dsc.font = &pixel_operator_mono_12; // Asegúrate que esta fuente existe
                                            // en custom_fonts.h
