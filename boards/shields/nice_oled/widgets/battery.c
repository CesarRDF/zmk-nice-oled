...
static void animation_smart_battery_on(lv_obj_t *canvas) {
    // 1. Limpiar la imagen estática si existe
    if (art2) {
        lv_obj_del(art2);
        art2 = NULL;
    }

    // 2. CORRECCIÓN DE MEMORIA: Solo crear si no existe
    if (!art) {
        art = lv_animimg_create(canvas);
        lv_obj_center(art);

        // Casting necesario para lv_animimg
        lv_animimg_set_src(art, (const void **)crystal_imgs_test, 16);
        lv_animimg_set_duration(art, CONFIG_NICE_OLED_GEM_ANIMATION_MS);
        lv_animimg_set_repeat_count(art, LV_ANIM_REPEAT_INFINITE);
        lv_animimg_start(art);
...
static void animation_smart_battery_off(lv_obj_t *canvas) {
    // 1. Limpiar la animación si existe
    if (art) {
        lv_obj_del(art);
        art = NULL;
    }

    // 2. CORRECCIÓN DE MEMORIA Y LVGL 9
    if (!art2) {
        art2 = lv_image_create(canvas); // lv_image_create en v9
